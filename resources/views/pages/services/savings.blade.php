<section class="savings">
    <div class="savings-content">
        <div class="savings-header">
            <h2 class="savings-title">Savings</h2>
            <p class="savings-description">
                Build your financial future with our diverse savings options, designed for security and growth.
            </p>
        </div>

        <div class="savings-card-container">
            <div class="group-row">
                <div class="savings-card">
                    <div class="card-inner">
                        <div class="card-front">
                            <h3 class="savings-name">SHARE CAPITAL</h3>
                        </div>
                        <div class="card-back">
                            <p>Earn dividends and strengthen your membership ownership.</p>
                        </div>
                    </div>
                </div>

                <div class="savings-card">
                    <div class="card-inner">
                        <div class="card-front">
                            <h3 class="savings-name">SAVINGS DEPOSIT</h3>
                        </div>
                        <div class="card-back">
                            <p>Secure your funds with easy access and steady growth.</p>
                        </div>
                    </div>
                </div>
            </div>

            <div class="group-row">
                <div class="savings-card">
                    <div class="card-inner">
                        <div class="card-front">
                            <h3 class="savings-name">KIDDIE SAVERS</h3>
                        </div>
                        <div class="card-back">
                            <p>Teach kids the value of saving with fun rewards.</p>
                        </div>
                    </div>
                </div>

                <div class="savings-card">
                    <div class="card-inner">
                        <div class="card-front">
                            <h3 class="savings-name">TIME DEPOSIT</h3>
                        </div>
                        <div class="card-back">
                            <p>Enjoy higher interest with fixed-term savings.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
